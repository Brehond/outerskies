...
def create_task_with_fallback(task_func, *args, **kwargs):
    """Create task with enhanced fallback (backward compatibility)"""
    return enhanced_celery_manager.create_task_with_enhanced_fallback(
        task_func, 'chart_generation', kwargs
    )


def get_task_status(task_id: str) -> Optional[Dict[str, Any]]:
    """Get task status (backward compatibility)"""
    return enhanced_celery_manager.get_task_status(task_id)
...
def create_task_with_fallback(task_func, args=None, kwargs=None, user=None, 
    ...
    try:
        ...
        if is_celery_available():
            ...
            try:
                ...
                logger.info(f"Returning sync fallback result: {{'success': True, ...}}")
                return {
                    'success': True,
                    'task_id': task_id,
                ...
                logger.info(f"Returning sync error result: {{'success': False, ...}}")
                return {
                    'success': False,
                    'task_id': task_id,
                    'status': 'FAILURE',
    ...
    logger.error(f"This should never happen - all code paths should return a result")
    return {
        'success': False,
